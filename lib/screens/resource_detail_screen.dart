...
class _ResourceDetailScreenState extends State<ResourceDetailScreen> {
  ...
  Future<void> _checkFavoriteStatus() async {
    try {
      final ids = await _favoriteService.fetchFavoriteIds();
      if (mounted) {
        setState(() => _isFavorite = ids.contains(widget.resourceId));
      }
    } catch (_) {
      // Non-critical — the heart just won't be pre-filled.
    }
  }
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      if (wasFavorite) {
        await _favoriteService.removeFavorite(widget.resourceId);
      } else {
        await _favoriteService.addFavorite(widget.resourceId);
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isFavorite = wasFavorite); // revert on failure
  ...
  void _openUploaderProfile() {
    final uploadedBy = resource?.uploadedBy;
    if (uploadedBy == null) return;
    Get.to(() => ProfilePage(userId: uploadedBy));
  }
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(
              ...
              color: _isFavorite
                  ? Colors.redAccent
                  : (isDark ? Colors.grey[400] : _primaryColor),
            ),
          ),
        ],
      ),
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // Uploaded by — tap to view their profile.
                            InkWell(
                              onTap: _openUploaderProfile,
                              borderRadius: BorderRadius.circular(8),
                              child: Padding(
                                padding: const EdgeInsets.symmetric(
                                  vertical: 4,
